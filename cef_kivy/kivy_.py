...
class BrowserLayout(MDBoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        layout.spacing = 20
        layout.add_widget(
            MDRectangleFlatButton(text="Back",
                                  on_press=self.browser_widget.go_back))
        layout.add_widget(
            MDRectangleFlatButton(text="Forward",
...
class CefBrowser(Widget):
    ...
    def __init__(
            self,
            start_url='https://www.missionjuno.swri.edu/junocam/processing',
            **kwargs):
    ...
    def devtools(self, *_):
        ...
        if "enable-begin-frame-scheduling" in g_switches:
            ...
            popup = Popup(title='DevTools INFO',
    ...
    def on_touch_down(self, touch, *kwargs):
        ...
        if touch.is_double_tap:
            ...
            self.browser.SendMouseClickEvent(touch.x,
                                             y,
                                             cef.MOUSEBUTTON_RIGHT,
                                             mouseUp=False,
                                             clickCount=1)
            self.browser.SendMouseClickEvent(touch.x,
                                             y,
    ...
    def on_touch_move(self, touch, *kwargs):
        ...
        self.browser.SendMouseMoveEvent(touch.x,
                                        y,
                                        mouseLeave=False,
                                        modifiers=modifiers)
        if self.is_drag:
            ...
            if self.is_inside_web_view(touch.x, y):
                if self.is_drag_leave:
                    ...
                    self.browser.DragTargetDragEnter(self.drag_data, touch.x, y,
    ...
    def update_drag_icon(self, x, y):
        if self.is_drag:
            if self.drag_icon:
                ...
                abuffer = image.GetAsBitmap(1.0, cef.CEF_COLOR_TYPE_BGRA_8888,
...
class ClientHandler:
    ...
    def OnLoadingStateChange(self, is_loading, **_):
        ...
        if self.load_start_time:
            print("[kivy_.py] OnLoadingStateChange: load time = {time}".format(
                time=time.time() - self.load_start_time))
    ...
    def OnPaint(self, element_type, paint_buffer, **_):
        ...
        self.browserWidget.texture.blit_buffer(paint_buffer,
                                               colorfmt='bgra',
                                               bufferfmt='ubyte')
    ...
    def StartDragging(self, drag_data, x, y, **_):
        ...
        print("[kivy_.py] ~~ DragTargetDragEnter")
        self.browserWidget.browser.DragTargetDragEnter(drag_data, x, y,
                                                       cef.DRAG_OPERATION_EVERY)
